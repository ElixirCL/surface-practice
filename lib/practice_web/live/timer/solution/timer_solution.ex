defmodule PracticeWeb.Live.TimerSolution do
  ...
  defp seconds_to_string(seconds) do
    "#{rem(seconds, @seconds_in_hour) |> div(@seconds_in_minute) |> pad_number()}:#{rem(seconds, @seconds_in_hour) |> rem(@seconds_in_minute) |> pad_number()}"
  end
  ...
  def mount(_params, _session, socket) do
    {:ok,
     assign(socket, :timer, :timer.send_interval(@one_second_in_milliseconds, self(), :tick))}
  end
